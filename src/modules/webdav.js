...
export function gatherAllSettingsForBackup() {
  const settings = {};
  const allKeys = [ ...Object.values(UI_TOGGLE_KEYS), CONFIG_KEY_USERS, CONFIG_KEY_KEYWORDS, CONFIG_KEY_CATEGORIES, CONFIG_KEY_PANEL_POS, CONFIG_KEY_BLOCK_OLD_POSTS_ENABLED, CONFIG_KEY_BLOCK_OLD_POSTS_DAYS ];
  allKeys.forEach(key => { settings[key] = GM_getValue(key); });
  return settings;
}
